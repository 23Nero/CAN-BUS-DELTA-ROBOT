...
static volatile double in_angel, dec_angel;
static volatile double angel = 0;
static volatile int checkTurn = 0;

void turnAngle(double angel);
...
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
  ...
  if (RxHeader.StdId == 0x001)
  {
		...
		in_angel = (int8_t)RxData[0];
		dec_angel = (int8_t)RxData[1];
		
		/*Calculate angel*/
		angel = in_angel + (dec_angel / 100);
		checkTurn = 1;
  }
}
...
static void MX_CAN_Init(void)
{
  ...
  canfilterconfig.FilterMaskIdHigh = 0x7FE << 5;
